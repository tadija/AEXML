...
class AEXMLTests: XCTestCase {
    ...
    func URLForResource(fileName: String, withExtension ext: String) -> URL {
        if let url = Bundle(for: AEXMLTests.self)
            ...
            #if BAZEL
            guard let buildWorkspace = ProcessInfo.processInfo.environment["BUILD_WORKSPACE_DIRECTORY"]
            else { fatalError("BUILD_WORKSPACE_DIRECTORY not provided!") }
            return URL(fileURLWithPath: buildWorkspace + "/Tests/AEXMLTests/Resources/\(fileName).\(ext)")
            #else
